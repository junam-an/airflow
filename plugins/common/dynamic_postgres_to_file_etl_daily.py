...
def create_postgres_to_file_meta_v2_tasks(
    dag_id: str,
    meta_postgres_conn_id: str = DEFAULT_META_POSTGRES_CONN_ID,
):
    """
    PostgreSQL -> File Meta V2 기반 ETL 공통 Task 생성 함수.

    V2 특징:
    - ETL_PARAM INSERT 없음
    - 기존 ETL_PARAM 최신 값을 기준으로 etl_meta_db_to_file.input_param UPDATE만 수행
    """
    ...
    def get_table_configs():
        ...
        print(f"Loaded PostgreSQL to File V2 ETL configs. dag_id={dag_id}")
    ...
    def run_etl(table_config: dict, **context):
        ...
        if not raw_source_exec_sql:
            raise ValueError(
                f"{raw_target_file_name}: source_exec_sql is empty"
            )

        if raw_target_file_type not in ("json", "csv", "text"):
            raise ValueError(
                f"{raw_target_file_name}: invalid target_file_type "
                f"[{raw_target_file_type}]"
            )

        if not raw_target_file_dir:
            raise ValueError(
                f"{raw_target_file_name}: target_file_dir is empty"
            )

        try:
            if not source_conn_name:
                raise ValueError(
                    f"{raw_target_file_name}: "
                    f"config_option.SOURCE_CONN_NAME is empty"
                )
