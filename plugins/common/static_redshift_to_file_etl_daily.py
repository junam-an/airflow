...
def build_vertica_conn_kwargs_from_env(conn_id: str) -> dict:
    ...
    kwargs = {
        "host": get_env_value(conn_id, "HOST"),
        "port": get_env_value(conn_id, "PORT", "5433"),
        "database": (
...
def get_single_table_config(
    ...
    WHERE etl_meta_db_to_file.dag_id = b.dag_id
      AND etl_meta_db_to_file.task_name = b.task_name
    """
    select_meta_sql = """
    SELECT
        task_name,
...
def run_redshift_to_file_etl(
    ...
    **context,
    ...
    try:
        ...
        source_hook = get_redshift_hook(source_conn_name)
...
# ****** airflow 전용 ******
def create_redshift_to_file_task(
    dag_id: str,
    task_name: str,
    ...
    if airflow_task is None:
        raise RuntimeError(
            "create_redshift_to_file_task() requires Airflow. "
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run static Redshift to file ETL without Airflow."
    )
