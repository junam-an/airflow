...
def build_redshift_conn_kwargs_from_env(conn_id: str) -> dict:
    ...
    kwargs = {
        ...
        "database": (
            get_env_value(conn_id, "DATABASE")
            or get_env_value(conn_id, "DBNAME")
        ),
        "user": get_env_value(conn_id, "USER"),
        "password": get_env_value(conn_id, "PASSWORD"),
    }
...
class StandaloneRedshiftHook:
    ...
    def get_conn(self):
        ...
        return redshift_connector.connect(
            **build_redshift_conn_kwargs_from_env(self.redshift_conn_id)
        )
...
class StandaloneVerticaHook:
    ...
    def get_conn(self):
        ...
        return vertica_python.connect(
            **build_vertica_conn_kwargs_from_env(self.vertica_conn_id)
        )
...
def get_single_table_config(
    ...
    WHERE etl_meta_file_to_db.dag_id = b.dag_id
      AND etl_meta_file_to_db.task_name = b.task_name
    """
    select_meta_sql = """
    SELECT
        task_name,
...
def run_file_to_vertica_etl(
    ...
    **context,
    ...
    try:
        ...
        target_hook = get_vertica_hook(target_conn_name)
        ...
        try:
            ...
            if stg_load_row_count > 0:
                ...
                update_join_condition_sql = " AND ".join(
                    [f"{target_table}.{pk} = s.{pk}" for pk in pk_columns]
                )
                ...
                if non_pk_columns:
                    ...
                    update_sql = f"""
                        ...
                        SET {update_set_sql}
                        FROM {stg_table} s
                        WHERE {update_join_condition_sql}
                    """
                else:
                    update_sql = None
                ...
                delete_join_condition_sql = " AND ".join(
                    [f"{target_table}.{pk} = s.{pk}" for pk in pk_columns]
                )
                ...
                delete_sql = f"""
                    ...
                    WHERE EXISTS (
                        SELECT 1
                        FROM {stg_table} s
                        WHERE {delete_join_condition_sql}
                    )
                """
...
# ****** airflow 전용 ******
def create_file_to_vertica_task(
    dag_id: str,
    task_name: str,
    ...
    if airflow_task is None:
        raise RuntimeError(
            "create_file_to_vertica_task() requires Airflow. "
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run static file to Vertica ETL without Airflow."
    )
