...
COLUMN_COMMENTS = {
    "etl_meta_db_to_db": {
        ...
        "config_option": "Airflow Connection 선택 시 자동 생성",
        "create_dt": "생성 일자",
        "disable_dt": "비활성화 일자",
        "update_dt": "최종 수정 일자",
...
META_TABLES = {
    "db_to_db": {"label": "DB → DB", "table": "etl_meta_db_to_db"},
    "db_to_file": {"label": "DB → File", "table": "etl_meta_db_to_file"},
    "file_to_db": {"label": "File → DB", "table": "etl_meta_file_to_db"},
}
...
class EtlMetaView(BaseView):
    ...
    def _get_default_input_param(self) -> str:
        hook = PostgresHook(postgres_conn_id=META_POSTGRES_CONN_ID)
        sql = """
        WITH BASE_PARAM AS
        (
        ...
        if not row or row[0] is None:
            raise Exception("ETL_CALENDAR 기준 기본 input_param 생성 결과가 없습니다.")
        return str(row[0])

    def _get_airflow_connections(self) -> list[dict]:
        session = Session()
        try:
            ...
            for conn in connections:
                result.append(
                    {
                        "conn_id": conn.conn_id or "",
                        "conn_type": conn.conn_type or "",
                        "description": getattr(conn, "description", "") or "",
                        "host": conn.host or "",
                        "port": "" if conn.port is None else str(conn.port),
                    }
                )
            return result
        finally:
    ...
    def _get_connection_by_id(self, conn_id: str) -> dict | None:
    ...
    def _build_connection_option_html(self, connections: list[dict], selected_conn_id: str = "") -> str:
        option_html = '<option value="">선택</option>'
        for conn in connections:
            conn_id = conn.get("conn_id") or ""
            conn_type = conn.get("conn_type") or ""
            host = conn.get("host") or ""
            port = conn.get("port") or ""
            selected = "selected" if conn_id == selected_conn_id else ""
            label_parts = [conn_id]
            if conn_type:
                label_parts.append(f"type={conn_type}")
            if host:
                label_parts.append(f"host={host}")
            if port:
                label_parts.append(f"port={port}")
            label = " / ".join(label_parts)
            option_html += f'''
            <option value="{escape(conn_id, quote=True)}" {selected}>{escape(label)}</option>
            '''
        return option_html

    def _build_config_option_from_connections(
        self,
        source_conn_id: str | None,
        target_conn_id: str | None,
    ) -> str:
        source_conn = self._get_connection_by_id(source_conn_id or "")
        target_conn = self._get_connection_by_id(target_conn_id or "")
        result = {}
        if source_conn:
            result.update(
                {
                    "SOURCE_DRIVER_TYPE": source_conn["conn_type"],
                    "SOURCE_CONN_NAME": source_conn["conn_id"],
                    "SOURCE_DESCRIPTION": source_conn["description"],
                    "SOURCE_DB_HOST": source_conn["host"],
                    "SOURCE_DB_PORT": source_conn["port"],
                }
            )
        if target_conn:
            result.update(
                {
                    "TARGET_DRIVER_TYPE": target_conn["conn_type"],
                    "TARGET_CONN_NAME": target_conn["conn_id"],
                    "TARGET_DESCRIPTION": target_conn["description"],
                    "TARGET_DB_HOST": target_conn["host"],
                    "TARGET_DB_PORT": target_conn["port"],
                }
            )
        if not result:
            return ""
    ...
    def _build_order_sql(self, sort_specs: list[tuple[str, str]], column_names: list[str]) -> str:
        if sort_specs:
            return "ORDER BY " + ", ".join([f'"{col}" {direction.upper()}' for col, direction in sort_specs])
    ...
    def _build_sort_header(self, meta_type, column_name, sort_specs, current_sort, search_column, search_text):
        query_string = self._build_sort_query_string(meta_type, search_column, search_text, current_sort, column_name)
        sort_mark = self._get_sort_mark(sort_specs, column_name)
        return f'''
        <th><a class="sort-link" href="/etl-meta/?{escape(query_string, quote=True)}">{escape(column_name)}{escape(sort_mark)}</a></th>
        '''
    ...
    def _render_tabs(self, selected_meta_type):
        ...
        for meta_type, info in META_TABLES.items():
            ...
            html += f'<a class="tab {active_class}" href="/etl-meta/?meta_type={escape(meta_type)}">{escape(label)}</a>'
        return html
    ...
    def _build_search_area(self, meta_type, column_names, search_column, search_text):
        ...
        for col in column_names:
            ...
            option_html += f'<option value="{escape(col)}" {selected}>{escape(col)}</option>'
        return f'''
        <form class="search-box" method="get" action="/etl-meta/">
            ...
            <div class="search-row">
                <div class="search-item">
                    <label>항목</label>
                    <select name="search_column"><option value="">전체</option>{option_html}</select>
                </div>
                <div class="search-keyword">
                    <label>검색</label>
                    <input type="text" name="search_text" value="{escape(search_text)}" placeholder="예: customer 또는 =customer">
                </div>
            ...
            <div class="search-help">일반 검색: <b>LIKE</b> 검색 / 검색어 앞에 <b>=</b> 입력 시 정확히 일치 검색</div>
        </form>
        '''
    ...
    def _build_search_sql(self, column_names, search_column, search_text):
        ...
        if search_column and search_column in column_names:
            return f'WHERE "{search_column}"::text {operator} %s', [value]
        return "WHERE " + " OR ".join([f'"{col}"::text {operator} %s' for col in column_names]), [value] * len(column_names)
    ...
    def _get_flow_rows(self, table_name: str, dag_id: str) -> list[dict]:
        ...
        flow_columns = [
            "task_name", "exec_seq", "source_table", "target_table", "load_option", "enable_yn",
            "target_pre_sql", "target_post_sql", "target_pre_cmd", "target_post_cmd", "source_pre_cmd",
        ]
        select_items = []
        ...
        return [dict(zip(flow_columns, record)) for record in records]
    ...
    def _build_flow_type_buttons(self, selected_meta_type: str, selected_dag_id: str | None = None) -> str:
        ...
        for meta_type, info in META_TABLES.items():
            ...
            html += f'<a class="tab {active_class}" href="/etl-meta/flow?{escape(urlencode(query_params), quote=True)}">{escape(info["label"])}</a>'
        return html
    ...
    def _build_flow_dag_select(self, meta_type: str, dag_ids: list[str], selected_dag_id: str) -> str:
        ...
        for dag_id in dag_ids:
            ...
            option_html += f'<option value="{escape(dag_id, quote=True)}" {selected}>{escape(dag_id)}</option>'
        return f'''
        <form class="search-box" method="get" action="/etl-meta/flow">
            ...
            <div class="search-row">
                <div class="search-keyword">
                    <label>dag_id</label>
                    <select name="dag_id">{option_html}</select>
                </div>
                <div class="search-button"><label>&nbsp;</label><button class="btn" type="submit">FLOW 조회</button></div>
            </div>
        </form>
        '''
    ...
    def _build_mermaid_flow(self, flow_rows: list[dict]) -> str:
        ...
        for idx, row in enumerate(flow_rows):
            ...
            label = "<br/>".join([f"{escape(k)}: {self._mermaid_escape(v)}" for k, v in label_items])
    ...
    def flow(self):
        ...
        no_dag_message = ""
        if not dag_ids:
            no_dag_message = f'<div class="error-message">{escape(META_SCHEMA)}.{escape(table_name)} 테이블에 조회 가능한 dag_id가 없습니다.</div>'
        html = f'''
        <html>
        <head>
            <style>{self._common_css()} .mermaid {{ min-width: 900px; }} pre.flow-source {{ margin-top: 16px; padding: 12px; background-color: #f7f7f7; border: 1px solid #ddd; border-radius: 6px; white-space: pre-wrap; font-size: 12px; }}</style>
            <script src="https://cdn.jsdelivr.net/npm/mermaid@10/dist/mermaid.min.js"></script>
            <script>
                if (window.mermaid) {{
                    mermaid.initialize({{ startOnLoad: true, securityLevel: 'loose', theme: 'default', flowchart: {{ htmlLabels: true, curve: 'basis' }} }});
                }}
            </script>
        </head>
        <body>
            <div class="page">
                {top_buttons_html}
                <h2>ETL Flow</h2>
                <div class="tabs">{flow_type_buttons_html}</div>
                <h3>{escape(META_SCHEMA)}.{escape(table_name)}</h3>
                {dag_select_html}
                {no_dag_message}
                <div class="flow-help">선택한 FLOW 유형과 dag_id 기준으로 exec_seq 오름차순 순차 FLOW를 표시합니다. enable_yn = N 인 TASK는 회색으로 표시됩니다.</div>
                <div class="flow-panel"><div class="mermaid">{escape(mermaid_text)}</div></div>
                <details><summary>Mermaid source 보기</summary><pre class="flow-source">{escape(mermaid_text)}</pre></details>
            </div>
        </body>
        </html>
        '''
    ...
    def delete(self):
        ...
        delete_sql = f"DELETE FROM {META_SCHEMA}.{table_name} WHERE {row_key_where_sql} IN ({placeholders})"
        hook = PostgresHook(postgres_conn_id=META_POSTGRES_CONN_ID)
    ...
    def disable(self):
        ...
        placeholders = ", ".join(["%s"] * len(selected_rows))
        update_sql = f"UPDATE {META_SCHEMA}.{table_name} SET {', '.join(set_sql_list)} WHERE {row_key_where_sql} IN ({placeholders})"
        hook = PostgresHook(postgres_conn_id=META_POSTGRES_CONN_ID)
    ...
    def save(self):
        ...
        try:
            ...
            for row_idx, row_key in enumerate(row_keys):
                ...
                update_sql = f"UPDATE {META_SCHEMA}.{table_name} SET {', '.join(set_cols)} WHERE {row_key_where_sql} = %s"
    ...
    def list(self):
        ...
        where_sql, search_params = self._build_search_sql(column_names, search_column, search_text)
        order_sql = self._build_order_sql(sort_specs, column_names)
        rows = hook.get_records(
            f"""
            SELECT {row_key_select_sql}, {select_cols}
            FROM {META_SCHEMA}.{table_name}
            {where_sql}
            {order_sql}
            LIMIT 100
        ...
        top_buttons_html = self._render_top_buttons()
        search_area_html = self._build_search_area(meta_type, column_names, search_column, search_text)
        editable_column_names = {col["name"] for col in self._get_editable_columns(columns)}
        header_html = '<th style="width: 40px; text-align: center;"><input type="checkbox" onclick="toggleAllRows(this)"></th>'
        header_html += "".join(
            self._build_sort_header(meta_type, col, sort_specs, sort, search_column, search_text)
            for col in column_names
        )
        row_html = ""
            ...
            for col_idx, value in enumerate(row_values):
                ...
                if col_name in editable_column_names:
                    ...
                    <td class="editable-cell" ondblclick="startEdit(this)">
                        ...
                        <input class="cell-input" type="text" name="cell_{row_idx}_{escape(col_name, quote=True)}" value="{value_attr}" data-original="{value_attr}" data-row-index="{row_idx}" onblur="finishEdit(this)" onkeydown="handleCellKeydown(event, this)">
                    </td>
                    '''
                else:
                    row_html += f"<td>{escape(value_text)}</td>"
            row_html += "</tr>"
        csrf_token = generate_csrf()
        html = f'''
        <html>
        <head><style>{self._common_css()} .editable-cell {{ cursor: pointer; }} .editable-cell:hover {{ background-color: #fffbe6; }} .cell-input {{ display: none; width: 100%; min-width: 120px; padding: 5px; border: 1px solid #017cee; border-radius: 3px; font-size: 13px; }} .cell-view {{ display: inline-block; min-width: 20px; white-space: pre-wrap; }}</style></head>
        <body>
            <div class="page">
                {top_buttons_html}
                <h2>ETL Meta 관리</h2>
                ...
                <h3>{escape(META_SCHEMA)}.{escape(table_name)}</h3>
                {search_area_html}
                <a class="btn" href="/etl-meta/new?meta_type={escape(meta_type)}">신규 DAG 설정 등록</a>
                <a class="btn gray" href="/etl-meta/flow?meta_type={escape(meta_type)}">FLOW 보기</a>
                <h3>최근 등록된 설정</h3>
                <form method="post" id="metaTableForm">
                    <input type="hidden" name="csrf_token" value="{csrf_token}">
                    <div class="table-actions">
                        <button class="btn" type="submit" formaction="/etl-meta/save?meta_type={escape(meta_type)}" onclick="return confirm('변경한 내용을 저장하시겠습니까?');">저장</button>
                        <button class="btn warning" type="submit" formaction="/etl-meta/disable?meta_type={escape(meta_type)}" onclick="return confirm('선택한 ROW를 비활성화 하시겠습니까?');">선택 ROW 비활성화</button>
                        <button class="btn danger" type="submit" formaction="/etl-meta/delete?meta_type={escape(meta_type)}" onclick="return confirm('선택한 ROW를 삭제하시겠습니까? 삭제 후 복구할 수 없습니다.');">선택 ROW 삭제</button>
                    </div>
                    <div class="edit-help">컬럼 값을 더블 클릭하면 수정할 수 있습니다. 수정 후 저장 버튼을 누르면 변경됩니다.</div>
            ...
            function finishEdit(input) {{
                ...
                if (input.value !== input.dataset.original && changedInput) {{ changedInput.value = "Y"; td.classList.add("edited-cell"); }}
            }}
            function handleCellKeydown(event, input) {{
                if (event.key === "Enter") {{ event.preventDefault(); finishEdit(input); }}
                if (event.key === "Escape") {{ event.preventDefault(); input.value = input.dataset.original; finishEdit(input); }}
            }}
            </script>
        </body>
        </html>
    ...
    def new(self):
        ...
        connections = self._get_airflow_connections()
        selected_source_conn_id = request.form.get("source_conn_id_select", "").strip()
        selected_target_conn_id = request.form.get("target_conn_id_select", "").strip()

        if request.method == "POST":
            for col in insert_columns:
                name = col["name"]
                if name == "config_option":
                    continue
            ...
            if not self._validate_required_values(form_values):
                ...
                if "column_mapping" in form_values and column_mapping_mode == "1:1":
                    form_values["column_mapping"] = self._convert_column_mapping_1_to_1(form_values.get("column_mapping"))

                if "input_param" in insert_column_names:
                    raw_input_param = form_values.get("input_param", "")
                    if not raw_input_param or not raw_input_param.strip():
                        form_values["input_param"] = self._get_default_input_param()

                if "config_option" in insert_column_names:
                    if meta_type == "db_to_db":
                        if not selected_source_conn_id:
                            error_message = "SOURCE_CONN_NAME을 선택해야 합니다."
                        elif not selected_target_conn_id:
                            error_message = "TARGET_CONN_NAME을 선택해야 합니다."
                        else:
                            form_values["config_option"] = self._build_config_option_from_connections(selected_source_conn_id, selected_target_conn_id)
                    elif meta_type == "db_to_file":
                        if not selected_source_conn_id:
                            error_message = "SOURCE_CONN_NAME을 선택해야 합니다."
                        else:
                            form_values["config_option"] = self._build_config_option_from_connections(selected_source_conn_id, None)
                    elif meta_type == "file_to_db":
                        if not selected_target_conn_id:
                            error_message = "TARGET_CONN_NAME을 선택해야 합니다."
                        else:
                            form_values["config_option"] = self._build_config_option_from_connections(None, selected_target_conn_id)
                ...
                if not error_message:
                    ...
                    if not col_names:
                        ...
                        insert_sql = f"INSERT INTO {META_SCHEMA}.{table_name} ({columns_sql}) VALUES ({placeholders})"
                        hook.run(insert_sql, parameters=tuple(values))
                        return redirect(f"/etl-meta/?meta_type={meta_type}")

        table_comments = COLUMN_COMMENTS.get(table_name, {})
        source_option_html = self._build_connection_option_html(connections, selected_source_conn_id)
        target_option_html = self._build_connection_option_html(connections, selected_target_conn_id)
        form_html = ""
        for col in insert_columns:
            name = col["name"]
            data_type = col["data_type"]
            ...
            if name == "column_mapping":
                ...
                '''
            elif name == "input_param":
                input_html = f'''
                <textarea name="{escape(name)}" rows="5" placeholder="미입력 시 ETL_CALENDAR 기준 기본 파라미터가 자동 생성됩니다." {required}>{escape(value)}</textarea>
                <div class="comment">미입력 시 $$P_BASE_DT, $$P_START_DT, $$P_END_DT, $$P_START_TM, $$P_END_TM, $$P_BEF_MAX_DT, $$P_MAX_DT 값이 자동 생성되어 저장됩니다.</div>
                '''
            elif name == "config_option":
                source_select_html = ""
                target_select_html = ""
                if meta_type in ("db_to_db", "db_to_file"):
                    source_select_html = f'''
                    <div class="connection-select-row">
                        <label>SOURCE_CONN_NAME</label>
                        <select name="source_conn_id_select">{source_option_html}</select>
                    </div>
                    '''
                if meta_type in ("db_to_db", "file_to_db"):
                    target_select_html = f'''
                    <div class="connection-select-row">
                        <label>TARGET_CONN_NAME</label>
                        <select name="target_conn_id_select">{target_option_html}</select>
                    </div>
                    '''
                input_html = f'''
                <div class="comment">Airflow Admin → Connections에 등록된 Conn Id를 선택하면 Conn Type, Conn Id, Description, Host, Port 정보를 기준으로 config_option JSON이 자동 생성되어 저장됩니다.</div>
                {source_select_html}
                {target_select_html}
                '''
            elif data_type in ("json", "jsonb"):
                input_html = f'<textarea name="{escape(name)}" rows="5" {required}>{escape(value)}</textarea>'
                ...
                input_html = f'<input type="text" name="{escape(name)}" value="{escape(value)}" {required}>'

            comment_html = ""
            if comment and name not in ("input_param", "config_option"):
                comment_html = f'<div class="comment">{escape(comment)}</div>'
            form_html += f'''
            <div class="form-row">
                <label>{required_mark}{escape(name)} <span class="type">({escape(data_type)})</span></label>
                {comment_html}
                {input_html}
            </div>
            '''
        ...
        tabs_html = self._render_tabs(meta_type)
        top_buttons_html = self._render_top_buttons()
        error_html = f'<div class="error-message">{escape(error_message)}</div>' if error_message else ""
        csrf_token = generate_csrf()
        html = f'''
        <html>
        <head><style>{self._common_css()} .connection-select-row {{ margin-bottom: 12px; }} .column-mapping-option-row {{ display: flex; align-items: center; gap: 8px; margin-bottom: 8px; }} .column-mapping-mode {{ width: 120px; }} .column-mapping-help {{ font-size: 12px; color: #555; }}</style></head>
        <body>
            <div class="page">
                {top_buttons_html}
    ...
    def _common_css(self) -> str:
        ...
        .tab { display: inline-block; padding: 9px 14px; margin-right: 6px; background-color: #333; color: white; text-decoration: none; border-radius: 4px; }
        .tab.active { background-color: #017cee; }
        .form-row { margin-bottom: 16px; padding-bottom: 12px; border-bottom: 1px solid #eee; }
        label { display: block; font-weight: bold; margin-bottom: 5px; }
        .required { color: #ff4d4f; font-weight: bold; margin-right: 4px; }
        .type { color: #777; font-weight: normal; font-size: 12px; }
        .comment { margin-bottom: 6px; padding: 7px 9px; background-color: #f5f5f5; border-left: 4px solid #017cee; color: #333; font-size: 12px; line-height: 1.4; }
        input, textarea, select { width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 4px; font-size: 13px; }
        .btn { display: inline-block; padding: 8px 14px; background-color: #017cee; color: white; border: none; border-radius: 4px; cursor: pointer; text-decoration: none; margin-bottom: 15px; }
        .btn.gray { background-color: #666; }
        .btn.warning { background-color: #faad14; color: #111; }
        .btn.danger { background-color: #d9363e; }
        .back { display: inline-block; margin-bottom: 15px; }
        .error-message { margin-bottom: 15px; padding: 12px 14px; background-color: #ffe8e8; border: 1px solid #ff9f9f; color: #b00020; border-radius: 4px; font-weight: bold; }
        .search-box { padding: 15px; margin-bottom: 20px; border: 1px solid #333; border-radius: 6px; background-color: #111; }
        .search-row { display: flex; gap: 12px; align-items: end; }
        .search-item { width: 260px; }
        .search-keyword { width: 420px; }
        .search-button { min-width: 180px; }
        .search-box label { display: block; margin-bottom: 5px; font-weight: bold; color: white; }
        .search-box input, .search-box select { width: 100%; padding: 8px; border: 1px solid #555; border-radius: 4px; background-color: #222; color: white; }
        .search-help { margin-top: 8px; color: #aaa; font-size: 12px; }
        .table-actions { margin-bottom: 10px; }
        .edit-help { margin-bottom: 10px; color: #888; font-size: 12px; }
        table { border-collapse: collapse; width: 100%; font-size: 13px; }
        th, td { border: 1px solid #ddd; padding: 8px; vertical-align: top; }
        th { background-color: #f5f5f5; color: #222; }
        .flow-panel { padding: 16px; border: 1px solid #ddd; border-radius: 8px; background-color: white; overflow-x: auto; }
        .flow-help { margin-bottom: 10px; color: #666; font-size: 12px; }
        """
