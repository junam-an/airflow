...
def create_file_to_odbc_meta_tasks(
    dag_id: str,
    meta_postgres_conn_id: str = DEFAULT_META_POSTGRES_CONN_ID,
    chunk_size: int = DEFAULT_CHUNK_SIZE,
):
    """
    File -> ODBC DB Meta 기반 ETL 공통 Task 생성 함수.

    사용 예:
        table_configs, run_etl_tasks = create_file_to_odbc_meta_tasks(
            dag_id=DAG_ID
        )
    ...
    def get_table_configs():
        ...
        print(f"Loaded File to ODBC ETL configs. dag_id={dag_id}")
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    print(
                        f"{stg_table} create skipped "
                        f"(already exists or create failed)"
                    )
                ...
                if source_file_type == "text":
                    if column_mapping:
                        if len(column_mapping) != 1:
                            raise ValueError(
                                f"{target_table}: "
                                f"text file requires exactly one column mapping"
                            )
                ...
                for file_path in matched_files:
                    ...
                    if source_file_type in ("csv", "json") and not source_columns:
                        raise ValueError(
                            f"{target_table}: "
                            f"no source columns detected from file [{file_path}]"
                        )
                    ...
                    if expected_source_columns is None:
                        ...
                        if source_columns != expected_source_columns:
                            raise ValueError(
                                f"{target_table}: "
                                f"source columns mismatch across files. "
                    ...
                    if all_rows:
                        ...
                        if normalized_rows:
                            ...
                            stg_count_sql = f"SELECT COUNT(*) FROM {stg_table}"
                            stg_count = target_hook.get_first(stg_count_sql)[0]
                            ...
                            print(
                                ...
                                f"file_rows={len(normalized_rows)} "
                ...
                if stg_load_row_count > 0:
                    ...
                    try:
                        ...
                        if load_option == "ui":
                            ...
                            print(
                                ...
                                f"MERGE completed (UI), total={stg_load_row_count}"
                            )
                            ...
                            print(
                                f"{stg_table} -> {target_table} "
                                f"INSERT completed (DI), total={stg_load_row_count}"
                            )
                            ...
                            print(
                                f"{stg_table} -> {target_table} "
                                f"INSERT completed (TI), total={stg_load_row_count}"
                            )
                            ...
                            print(
                                f"{stg_table} -> {target_table} "
                                f"INSERT completed (I), total={stg_load_row_count}"
                            )
                            ...
                            if not merge_update_sql:
                                print(
                                    f"{target_table}: "
                                    f"no non-pk columns to update, UPDATE skipped (U)"
                                )
                            else:
                    ...
                    print(
                        f"{target_table}: "
                        f"no rows read from matched files, target load skipped"
                    )
                    job_succeeded = True
