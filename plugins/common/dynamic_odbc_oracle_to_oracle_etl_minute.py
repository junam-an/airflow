...
def create_odbc_to_odbc_meta_tasks(
    dag_id: str,
    meta_postgres_conn_id: str = DEFAULT_META_POSTGRES_CONN_ID,
    chunk_size: int = DEFAULT_CHUNK_SIZE,
):
    """
    ODBC 기반 DB -> DB Meta ETL 공통 Task 생성 함수.

    사용 예:
        table_configs, run_etl_tasks = create_odbc_to_odbc_meta_tasks(
            dag_id=DAG_ID
        )
    ...
    def get_table_configs():
        ...
        print(f"Loaded ODBC to ODBC ETL configs. dag_id={dag_id}")
